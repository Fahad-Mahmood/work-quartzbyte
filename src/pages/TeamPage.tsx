...
import { Users, ChevronRight, ArrowUpDown } from 'lucide-react';
    ...
    <div className="flex flex-col gap-8 max-w-[1100px] mx-auto animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        <div className="flex items-stretch rounded-2xl overflow-hidden border border-outline-variant/20 shadow-sm bg-surface-container-lowest shrink-0">
          <div className="px-8 py-4 text-center">
            ...
            <p className="text-3xl font-headline font-extrabold text-on-surface">{members.length}</p>
          </div>
      ...
      {sorted.length === 0 ? (
        ...
        <div className="bg-surface-container-lowest rounded-2xl shadow-sm border border-outline-variant/10 overflow-hidden">
          {/* Header row */}
          <div className="grid grid-cols-[1fr_150px_1fr_150px_100px] gap-4 px-8 py-3 bg-surface-container border-b border-outline-variant/10">
          ...
          <div className="divide-y divide-outline-variant/10">
            {sorted.map(member => {
              ...
              return (
                <div
                  key={member.id}
                  className="grid grid-cols-[1fr_150px_1fr_150px_100px] gap-4 px-8 py-5 items-center hover:bg-surface-container-high/40 transition-colors"
                >
                  {/* Member */}
          ...
          <div className="px-8 py-4 border-t border-outline-variant/10 bg-surface-container-lowest/60">
            <p className="text-xs text-on-surface-variant font-medium">
              Showing <span className="font-bold text-on-surface">{sorted.length}</span> member{sorted.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
      )}
